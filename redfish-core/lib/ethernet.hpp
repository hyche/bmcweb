...
class OnDemandEthernetProvider
{
  private:
    ...
    const PropertiesMapType *extractInterfaceProperties(
        ...
        const std::string &interface, const GetManagedObjectsType &dbusData)
    {
        const auto &dbusObj = dbusData.find(objpath);
        if (dbusObj != dbusData.end())
        {
    ...
    inline const PropertiesMapType *
        extractInterfaceProperties(const std::string &objpath,
                                   ...
                                   const GetManagedObjectsType &dbusData)
    {
        const auto &dbusObj = sdbusplus::message::object_path{objpath};
        return extractInterfaceProperties(dbusObj, interface, dbusData);
    }
                                      ...
                                      EthernetInterfaceData &ethData)
    {
        ...
        const PropertiesMapType *statusProperties = extractInterfaceProperties(
            "/xyz/openbmc_project/network/" + ethifaceId,
            "xyz.openbmc_project.State.Decorator.OperationalStatus", dbusData);

        if (statusProperties != nullptr)
        {
            ethData.enabled =
                extractProperty<bool>(*statusProperties, "Functional");
        }

        // Extract data that contains MAC Address
        const PropertiesMapType *macProperties = extractInterfaceProperties(
            "/xyz/openbmc_project/network/" + ethifaceId,
            "xyz.openbmc_project.Network.MACAddress", dbusData);
        ...
        const PropertiesMapType *vlanProperties = extractInterfaceProperties(
            "/xyz/openbmc_project/network/" + ethifaceId,
            "xyz.openbmc_project.Network.VLAN", dbusData);
        ...
        const PropertiesMapType *ethProperties = extractInterfaceProperties(
            "/xyz/openbmc_project/network/" + ethifaceId,
            "xyz.openbmc_project.Network.EthernetInterface", dbusData);

        if (ethProperties != nullptr)
        {
            ethData.autoNeg = extractProperty<bool>(*ethProperties, "AutoNeg");
            ethData.speed =
        ...
        const PropertiesMapType *configProperties = extractInterfaceProperties(
            ...
            "xyz.openbmc_project.Network.SystemConfiguration", dbusData);

        if (configProperties != nullptr)
        {
            ethData.hostname =
                extractProperty<std::string>(*configProperties, "HostName");
            ethData.defaultGateway = extractProperty<std::string>(
                         ...
                         std::vector<IPv4AddressData> &ipv4Config)
    {
                    ...
                    static_cast<const std::string &>(objpath.first), pathStart))
            {
                ...
                if (interface != objpath.second.end())
                {
                    ...
                    if (ipv4Address.address != nullptr)
                    {
                        ...
                        ipv4Config.emplace_back(std::move(ipv4Address));
                    }
                }
            }
        }
         ...
         * PATCHes
         */
        std::sort(ipv4Config.begin(), ipv4Config.end());
    }
  ...
  public:
     ...
     * @param[in] ipIdx         Index of IP in input array that should be
     * deleted
     * @param[in] subnetMask    SubnetMask of new ipv4
     * @param[in] prefixLength  Prefix length for Dbus to create an ipv4.
     * @param[in] gateway       Gateway of new ipv4
     * @param[in] address       ipv4 address need to be created.
     * @param[io] asyncResp     Response object that will be returned to client
     *
     * @return None
     */
        ...
        auto createIpHandler =
            [ipIdx{std::move(ipIdx)}, asyncResp{std::move(asyncResp)},
             ...
             subnetMask{std::move(subnetMask)}](
                ...
                {
                    asyncResp->res.jsonValue["IPv4Addresses"][ipIdx] = {
                        {"Address", address},
...
class EthernetCollection : public Node
{
  ...
  private:
               ...
               const std::vector<std::string> &params) override
    {
        ...
        // for any Manager, not only hardcoded 'bmc'.
        std::string managerId = "bmc";
...
class EthernetInterface : public Node
{
  public:
                                ...
                                const std::shared_ptr<AsyncResp> &asyncResp)
    {
        ...
        if (inputVlanEnabled == true && inputVlanId != *ethData.vlanId)
        {
            // Change VLAN Id
  ...
  private:
                               ...
                               const std::shared_ptr<AsyncResp> &asyncResp)
    {
        ...
        auto handler = [asyncResp, value](const boost::system::error_code ec) {
            ...
            {
                asyncResp->res.jsonValue["InterfaceEnabled"] = value;
            }
        };
        ...
        crow::connections::systemBus->async_method_call(
            ...
            "Functional", sdbusplus::message::variant<bool>(value));
    }
                             ...
                             const std::shared_ptr<AsyncResp> &asyncResp)
    {
        if (input.is_string())
        {
            ...
            if (ethData.hostname == nullptr || newHostname != *ethData.hostname)
            {
                // Change hostname
                         ...
                         const std::vector<std::string> &params)
    {
        ...
        const std::string &main_object_path =
        ...
        {
            BMCWEB_LOG_ERROR << "Incorrect MACAddress value: not local address";
        ...
        crow::connections::systemBus->async_method_call(
            [&, main_object_path, process_name,
             dest_property{std::move(dest_property)},
             property_value](const boost::system::error_code error_code,
                             const GetManagedObjectsType &resp) {
                if (error_code)
                {
                    BMCWEB_LOG_ERROR << "Internal Error";
                    asyncResp->res.result(
                        boost::beast::http::status::internal_server_error);
                    return;
                }

                const auto &objpath = resp.find(
                    sdbusplus::message::object_path{main_object_path});
                if (objpath != resp.end())
                {
                    // Iterate for all interfaces available for specified
                    // ObjectPath.
                    for (auto &interface : objpath->second)
                    {
                        if (interface.first.find(process_name) !=
                            std::string::npos)
                        {
                            crow::connections::systemBus->async_method_call(
                                [&, main_object_path, process_name,
                                 ...
                                 property_value](
                                    ...
                                    {
                                        auto it =
                                            properties.find(dest_property);
                                        if (it != properties.end())
                                        {
                                            // Create the D-Bus variant for
                                            // D-Bus call.
    ...
    bool isValidMacAddress(const std::string &mac_address)
    {
        ...
        const unsigned char local_mac_address_mask =
            (0x01 << local_address_pos);
                         ...
                         const std::shared_ptr<AsyncResp> &asyncResp)
    {
        ...
        for (unsigned int entryIdx = 0; entryIdx < input.size(); entryIdx++)
        {
            ...
            {
                if (entryIdx < ipv4Data.size())
                {
                    // null on existing data indicates delete.
                    ethernetProvider.deleteIPv4(ifaceId, ipv4Data[entryIdx].id,
                                                entryIdx, asyncResp);
                }
                continue;
            }
            ...
            if (entryIdx < ipv4Data.size())
            {
                ...
                addressFieldValue = ipv4Data[entryIdx].address;
                subnetMaskFieldValue = &ipv4Data[entryIdx].netmask;
                ethernetProvider.ipv4VerifyIpAndGetBitcount(
                    *subnetMaskFieldValue, &subnetMaskAsPrefixLength);
                gatewayFieldValue = ipv4Data[entryIdx].gateway;
            }
            ...
            ethernetProvider.createIPv4(
                ifaceId, entryIdx, *subnetMaskFieldValue,
                subnetMaskAsPrefixLength, *gatewayFieldValue,
                           ...
                           const std::vector<IPv6AddressData> &ipv6Data)
    {
        ...
        if (ethData.enabled != nullptr)
          jsonResponse["InterfaceEnabled"] = *ethData.enabled;

        if (ethData.autoNeg != nullptr)
          jsonResponse["AutoNeg"] = *ethData.autoNeg;

        if (ethData.speed != nullptr)
            jsonResponse["SpeedMbps"] = *ethData.speed;

        if (ethData.macAddress != nullptr)
            jsonResponse["MACAddress"] = *ethData.macAddress;

        if (ethData.hostname != nullptr)
        ...
        if (!ipv6Data.empty())
        {
            ...
            for (const IPv6AddressData &ipv6Config : ipv6Data)
            {
               ...
               const std::vector<std::string> &params) override
    {
        ...
        ethernetProvider.getEthernetIfaceData(
            ...
            [&, ifaceId](const bool &success,
                ...
                {
                    res.jsonValue =
                        parseInterfaceData(ifaceId, ethData, ipv4Data, ipv6Data);
                }
                else
                {
                 ...
                 const std::vector<std::string> &params) override
    {
        ...
        ethernetProvider.getEthernetIfaceData(
            ifaceId,
            [&, ifaceId, patchReq = std::move(patchReq), params](
                ...
                res.jsonValue = parseInterfaceData(ifaceId, ethData, ipv4Data,ipv6Data);
                     ...
                     propertyIt != patchReq.end(); ++propertyIt)
                {
                    ...
                    else if (propertyIt.key() == "MacAddress")
                    {
                    ...
                    {
                        handleIPv4Patch(ifaceId, propertyIt.value(), ipv4Data,
                                        asyncResp);
                    }
        ...
        ethernetProvider.getEthernetIfaceData(
            ...
            [&, parentIfaceId,
             ifaceId](const bool &success, const EthernetInterfaceData &ethData,
                      const std::vector<IPv4AddressData> &ipv4Data,
                      const std::vector<IPv6AddressData> &ipv6Data) {
                if (success && ethData.vlanId != nullptr)
                {
